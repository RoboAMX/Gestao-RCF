...
# Importações do Leitor de Etiqueta (Câmera)
try:
...
with aba_recebimento:
    if st.session_state["perfil_atual"] == "Almoxarifado":
        ...
        with st.container():
            ...
            # A CÂMERA MÁGICA
            foto_camera = col_b1.camera_input("📷 Escanear Data Matrix")
        ...
        # LÓGICA DE PROCESSAMENTO DA FOTO!
        if foto_camera and leitor_ativo:
            with st.spinner("Analisando Código..."):
                ...
                if codigos:
                    ...
                    # Decodificador de SAP (Procura o material depois do prefixo 240)
                    if "240" in texto_bruto and len(texto_bruto) > 15:
                        busca_global = texto_bruto.split("240")[-1].strip()
                    else:
                        busca_global = texto_bruto
                    st.success(f"✅ Material Identificado: {busca_global}")
                else:
                    st.error("❌ Etiqueta não reconhecida. Tente aproximar ou melhorar a luz.")
        elif foto_camera and not leitor_ativo:
            st.error("⚠️ As bibliotecas de câmera não estão instaladas no servidor Nuvem. Adicione 'pylibdmtx' no requirements.txt e 'libdmtx0a' no packages.txt do GitHub.")
        ...
        if df_bruto.empty:
            ...
            if df_tela.empty:
                ...
                with area_botoes_expedicao:
                    ...
                    with col_btn1: st.info(f"🛒 **{qtd_carrinho}** itens no carrinho.")
                    with col_btn2:
                    ...
                    with col_btn4:
                        ...
                        if st.button(f"🚚 Gerar Lote e Despachar", type="primary", use_container_width=True):
                            ...
                            elif not itens_ja_despachados.empty: st.error("⚠️ Remova itens JÁ DESPACHADOS!")
...
# ------------------------------------------
# ABA 2: ACONDICIONAR E ABA 3 / 4 (MANTIDAS IGUAIS PARA POUPAR ESPAÇO)
# ------------------------------------------
# ... (O código das abas 2, 3 e 4 continua exatamente igual ao anterior)
